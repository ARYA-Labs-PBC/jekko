...
fn no_keys_opens_setup_screen_and_creates_template() -> Result<()> {
    ...
    let (workspace, _project, _, _, _, _) = prepare_workspace_no_config("project", "# new-user-setup\n")?;
    let page = spawn_jekko_with_size_env(
        ...
        30,
        "no-keys",
        &[("JEKKO_API_KEY", ""), ("JNOCCIO_DEVELOPER_KEY", ""), ("JEKKO_IGNORE_DEVELOPER_KEY", "1")],
    )?;
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            ...
            bail!("setup screen did not appear within {}ms", FIRST_VISIBLE_TIMEOUT.as_millis());
        }
        std::thread::sleep(Duration::from_millis(50));
    }

    page.expect_screen().to_contain_text("platform.openai.com/api-keys")?;
    page.expect_screen().to_contain_text("console.anthropic.com/settings/keys")?;

    let template = workspace.path().join(".jekko").join("jekko.env");
    assert!(template.exists(), "expected canonical key file to be created");
...
fn one_key_reaches_prompt_and_shows_auto() -> Result<()> {
    ...
    let (workspace, _project, _, _, _, _) = prepare_workspace_no_config("project", "# one-key-setup\n")?;
    let page = spawn_jekko_with_size_env(
        &workspace,
        &jekko,
        120,
        ...
        &[
            ("JEKKO_API_KEY", ""),
            ("JEKKO_MODEL_KEYS_CONTENT", "OPENAI_API_KEY=one-key-openai\n"),
        ],
    )?;

    page.wait_for_text("Auto", FIRST_VISIBLE_TIMEOUT).with_context(|| {
    ...
    page.expect_screen().not_to_contain_text("No model keys found")?;
    Ok(())
}
