...
async fn discover(args: &[String]) -> Result<(), String> {
    let run_root = value(args, "--run-root")
        .unwrap_or_else(|| ".jekko/daemon/paper-qbank/discovery".to_string());
    let query = value(args, "--query")
        .unwrap_or_else(|| "open access scientific paper hard answerable result".to_string());
...
fn make_work(args: &[String]) -> Result<(), String> {
    let bank = path_value(args, "--bank")
        .unwrap_or_else(|| PathBuf::from("crates/memory-benchmark/data/real-paper-bank"));
    let out = path_value(args, "--out")
        .unwrap_or_else(|| PathBuf::from(".jekko/daemon/paper-qbank/work.jsonl"));
...
fn pack_context_command(args: &[String]) -> Result<(), String> {
    ...
    let paper: PaperRecord = read_json(&input)?;
    let sections = value(args, "--sections")
        .unwrap_or_default()
        .split(',')
        .filter(|value| !value.is_empty())
        .map(str::to_string)
        .collect::<Vec<_>>();
    let safe_window = value(args, "--safe-window-tokens")
