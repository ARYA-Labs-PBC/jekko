...
//!
//! Bun's `migrationHash` (see `packages/jekko/src/storage/migration-repair.ts`
...
//! must produce byte-identical output, otherwise a Bun-written
...
fn hash_matches_known_first_migration() {
    ...
    assert_eq!(
        ...
        "hash drift from Bun migrationHash() for {}",
        path
    );
}
