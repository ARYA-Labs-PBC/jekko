...
### 2026-05-13T03:22:21Z — codex — ZYAL feature-maker runbooks

- Branch: `codex/zyal-feature-maker-examples`
- Commit: `f08dff042` (`docs: add ZYAL feature-maker runbooks`)
- PR: https://github.com/neverhuman/jekko/pull/2
- Files changed: `docs/ZYAL/FEATURE_MAKER.md`, `docs/ZYAL/CHANGELOG.md`, `docs/ZYAL/SPEC.md`, `docs/ZYAL/examples/README.md`, `docs/ZYAL/examples/24-semantic-feature-maker-simple.zyal`, `docs/ZYAL/examples/25-semantic-feature-maker-advanced.zyal`, `docs/ZYAL/examples/26-semantic-feature-maker-insane.zyal`, `packages/jekko/src/agent-script/parser.test.ts`, `packages/jekko/src/agent-script/schema-spec.test.ts`, `packages/jekko/src/agent-script/schema-spec.ts`
- Validation: `rtk bun --cwd packages/jekko ./script/generate-zyal-spec.ts --write`; `rtk bun --cwd packages/jekko ./script/generate-zyal-spec.ts --check`; `rtk bun --cwd packages/jekko test src/agent-script/schema-spec.test.ts src/agent-script/parser.test.ts`; `rtk cargo test --manifest-path crates/tuiwright-jekko-unlock/Cargo.toml all_tracked_zyal_files_parse_and_preview`; `rtk just fast`; `git diff --check`
- Skipped checks: none
- Notes: no ZYAL schema bump; the remaining repository dirty state is unrelated to this change set and was left untouched.
