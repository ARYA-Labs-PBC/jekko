...
// Zod discriminated union kept for the historical Hono OpenAPI path.
...
export const toModelMessagesEffect = Effect.fnUntraced(function* (
  ...
  //
  // Only apply this alternative if the model actually supports image input -
...
export function page(input: { sessionID: SessionID; limit: number; before?: string }) {
  ...
  const where = before
    ? and(eq(MessageTable.session_id, input.sessionID), beforeCursor(before))
...
export * as MessageV2 from "./message"
